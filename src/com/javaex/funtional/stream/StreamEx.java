...
//		System.out.println(arrStream);
		...
		students.add(new Student("홍길동", 80, 50, 90));
		students.add(new Student("전우치", 80, 90, 80));
		students.add(new Student("임꺽정", 70, 65, 75));
...
//		testFilterSort(students);	// filter + sorted
	...
	private static void testForEach(int[] source, List<Student> lst) {
		...
		// forEach 사용
		Arrays.stream(source).forEach(n -> System.out.println(n + " "));
	...
	private static void testFilter(int[] arr, List<Student> lst) {
		...
		lst.stream().filter(obj -> obj.getAverage() >= 70).forEach(System.out::println);
	}
...
//			.sorted((obj1, obj2) -> obj1.getName().compareTo(obj2.getName()))	//	정렬 (오름차순)
				.sorted((obj1, obj2) -> -obj1.getName().compareTo(obj2.getName())).forEach(System.out::println);
	}
	...
	private static void testReduce(int[] arr) {
		...
		int total = Arrays.stream(arr).reduce(0, // 누산기 초기값
